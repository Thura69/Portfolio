...
const ProjectsCard:React.FC<projectCardProps> = ({id,project_name,description,date,image,video,github_link,demo_link,category}) => {
  return (
      <RevealWrapper origin='bottom' className='w-[32%] h-[460px]' duration={1000}>
          <div className=' group hover:translate-y-[-10px] duration-300 relative'>
  ...
  <source src={video} type="video/mp4"/>
              ...
              <div className="absolute opacity-0 group-hover:translate-y-[-10px] flex group-hover:opacity-100 duration-500 bottom-2 gap-3 right-2">
     ...
     <FaGithub />
          ...
          <div className='my-3'>
              ...
              <div className='flex gap-2'>
                  {
                      category?.map((e) => (<div key={e} className='  rounded-[18px] uppercase font-sans p-[6px]  text-[11px] my-2 bg-textgray inline-block'>{e}</div>))
                  }
              </div>
              <h2 className='text-[21px] font-bold text-text '>{project_name}</h2>
              <p className='text-textgray text-[12px] mt-1 '>{description}</p>
          </div>
   </RevealWrapper>
  )
}
