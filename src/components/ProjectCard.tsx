...
const ProjectCard: React.FC<projectCardProps> = ({ title, year, description, image, link, account, accountDetail, url, skills,vd, toPage = "" }) => {
  ...
  return (
      ...
      {/* <AnimatedTextWord classname='text-[#77818f] font-sans mt-6 text-[16px] font-light' text={description} /> */} 
              <Marquee className='h-[400px]  mt-[30px] overflow-clip  '>
        <div className=' overflow-hidden mx-4'>
          {
            vd?.[0] ?  <video  loop autoPlay  muted className='   w-[610px] h-[400px] rounded-xl group-hover:block' width="100%" height="auto" >
  <source src={image[0] as any} type="video/webm"/>
  </video>:  <Image src={image[0]} className='  overflow-hidden rounded-xl' width={610} height={400} alt='win' />
          }
               
              </div>
              <div className='overflow-hidden mx-4'>
          {
            vd?.[1] ?  <video  loop autoPlay  muted className='  w-[610px] h-[400px] rounded-xl group-hover:block' width="100%" height="auto" >
  <source src={image[1] as any} type="video/webm"/>
  </video>:  <Image src={image[1]} className='  overflow-hidden rounded-xl' width={610} height={400} alt='win' />
          }
              </div>
              <div className='overflow-hidden mx-4'>
                {
            vd?.[2] ?  <video  loop autoPlay  muted className='  w-[610px] h-[400px] rounded-xl group-hover:block' width="100%" height="auto" >
  <source src={image[2] as any} type="video/webm"/>
  </video>:  <Image src={image[2]} className='  overflow-hidden rounded-xl' width={610} height={400} alt='win' />
          }
              </div>
              <div className=' overflow-hidden mx-4'>
                  {
            vd?.[3] ?  <video  loop autoPlay  muted className='  w-[610px] h-[400px] rounded-xl group-hover:block' width="100%" height="auto" >
  <source src={image[3] as any} type="video/webm"/>
  </video>:  <Image src={image[3]} className='  overflow-hidden rounded-xl' width={610} height={400} alt='win' />
          }
              </div>
             </Marquee>
            </div>
